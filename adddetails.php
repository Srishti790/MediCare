...
<!-- Login Form --><br><br><br><br>
   ...
   <?php }
       ...
       while($row=mysqli_fetch_assoc($result)){
        $query="SELECT * FROM customer WHERE username='".$_POST['submit']."'";
        $results=mysqli_query($conn,$query);
        $resultcheck=mysqli_num_rows($results);
        if($resultcheck<1){
          echo '<h2>No user found/error occured</h2>';
        }
        else{
                while($res=mysqli_fetch_assoc($results)){?>
                <form action="includes/adddetailsact.inc.php" method="post">
                  <div class ="form-group">
                  <input class ="form-control" type="text" placeholder="Full Name" readonly="" value="<?php echo $res['fullname']?>" name="cname" required="" autocomplete="off">
                  </div>
                  <div class ="form-group">
                  <input class ="form-control" type="text" placeholder="UserName" readonly="" value="<?php echo $row['cname']?>" name="cname" required="" autocomplete="off">
                  </div>     
                  <div class ="form-group">
                  <input class ="form-control" type="text" placeholder="Type" readonly="" value="<?php echo $row['types']?>" name="type" required="" autocomplete="off">
                  </div>
                  <div class ="form-group">
